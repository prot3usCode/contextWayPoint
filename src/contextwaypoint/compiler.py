...
from contextwaypoint.validation import (
    discover_yaml_files,
    load_yaml,
...
def fill_missing_problem_uuids_in_documents(
    ...
    for _, root in documents:
        changed = assign_missing_problem_uuids(root, seen_problem_uuids) or changed
...
def compile_context(
    ...
    if fill_uuids:
        ...
        selected_input_file, root = documents[0]
        fill_missing_problem_uuids(root)

        filled_yaml_target = selected_input_file if in_place else output_yaml
        if filled_yaml_target is None:
            filled_yaml_target = default_filled_yaml_path(selected_input_file)

        write_yaml(filled_yaml_target, root)

    raise_for_validation_errors(validate_context_documents(documents))

    flat_index = add_source_order(flatten_documents(documents))
    raise_for_validation_errors(validate_compiled_index(flat_index))
    ...
    return f"Compiled {len(flat_index)} entries from {input_label} to {display_path(output_file)}"


def compile_source(source: Path, output_file: Path = DEFAULT_INDEX_FILE) -> str:
    if not source.exists():
        raise ValueError(f"{source} does not exist")

    if source.is_dir():
        return compile_context(None, source, output_file)

    return compile_context(source, None, output_file)


def target_path_for_filled_document(
    input_path: Path,
    source: Path,
    ...
    if source.is_dir():
        output_dir = output_path or source.with_name(f"{source.name}_filled")
        return output_dir / input_path.relative_to(source)

    if output_path is not None:
        return output_path

    return default_filled_yaml_path(input_path)


def fill_uuid_source(
    source: Path,
    ...
    if source.is_dir() and output_path is not None:
        suffix = output_path.suffix.lower()
        if suffix in YAML_SUFFIXES:
            raise ValueError("When filling a directory, --out must point to a directory")

    documents = load_documents(
        input_file=None if source.is_dir() else source,
        input_dir=source if source.is_dir() else None,
    )

    fill_missing_problem_uuids_in_documents(documents)
    raise_for_validation_errors(validate_context_documents(documents))

    written_files: list[Path] = []

    for input_path, root in documents:
        target_path = target_path_for_filled_document(
            input_path,
            source,
            output_path,
            in_place,
        )
        write_yaml(target_path, root)
...
def legacy_main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.fill_uuids:
        target = args.input if args.in_place else args.output_yaml
        if target is None and args.input is not None:
            target = default_filled_yaml_path(args.input)
        print(f"Wrote filled YAML to {display_path(target)}")
